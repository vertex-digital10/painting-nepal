...
function parseSegments(defaultCaseSensitive, route, start, node, dynamicListsToSort, parentInterpolation) {
	...
	if (!node || path.includes("$")) {
		...
		const tail = require_utils.last(interpolation);
...
function processRouteMasks(routeList, processedTree) {
	...
	processedTree.flatCache = require_sieve_cache.createSieveCache(1e3);
}
/**
...
function processRouteTree(routeTree, caseSensitive = false) {
	...
	function visit(route, start, parentNode, parentInterpolation) {
		...
		if (index !== 0 && route.path) {
			const trimmedFullPath = require_path.trimPathRight(route.fullPath);
	...
	return {
		processedTree: {
			segmentTree,
			singleCache: require_sieve_cache.createSieveCache(1e3),
			matchCache: require_sieve_cache.createSieveCache(1e3),
			flatCache: null,
			masksTree: null
		},
		routesById,
