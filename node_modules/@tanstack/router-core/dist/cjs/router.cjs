	...
	*/
	constructor(options, getStoreConfig) {
		...
		this.update = (newOptions) => {
			...
			if (!(_tanstack_router_core_isServer.isServer ?? this.isServer)) this.staticLocations = /* @__PURE__ */ new WeakMap();
			...
			if (rewriteChanged) {
				...
				this.rewrite = nextBasepath !== "/" && require_path.trimPath(nextBasepath) ? require_rewrite.rewriteBasepath(nextBasepath, this.options.caseSensitive, nextRewriteOption) : nextRewriteOption;
			}
			if (this.history) this.updateLatestLocation();
			if (this.options.routeTree !== this.routeTree || (_tanstack_router_core_isServer.isServer ?? this.isServer) && prevOptions?.caseSensitive !== this.options.caseSensitive) {
				this.routeTree = this.options.routeTree;
				let processRouteTreeResult;
				if (process.env.NODE_ENV !== "development" && (_tanstack_router_core_isServer.isServer ?? this.isServer) && globalThis.__TSR_CACHE__ && globalThis.__TSR_CACHE__.routeTree === this.routeTree && globalThis.__TSR_CACHE__.caseSensitive === this.options.caseSensitive) processRouteTreeResult = globalThis.__TSR_CACHE__.processRouteTreeResult;
				else {
					processRouteTreeResult = this.buildRouteTree();
					if (process.env.NODE_ENV !== "development" && (_tanstack_router_core_isServer.isServer ?? this.isServer) && globalThis.__TSR_CACHE__ === void 0) globalThis.__TSR_CACHE__ = {
		...
		this.buildRouteTree = () => {
			const result = require_new_process_route_tree.processRouteTree(this.routeTree, this.options.caseSensitive);
			if (this.options.routeMasks) require_new_process_route_tree.processRouteMasks(this.options.routeMasks, result.processedTree);
			return {
				...result,
				resolvePathCache: require_sieve_cache.createSieveCache(1e3)
			};
		};
			...
			const parse = ({ pathname, search, hash, href }, state) => {
				if (!this.rewrite && !/[ \x00-\x1f\x7f\u0080-\uffff]/.test(pathname)) {
					...
					return {
						...
						pathname: require_utils.decodePath(pathname),
						external: false,
						searchStr,
						search: require_utils.nullReplaceEqualDeep(previousLocation?.search, parsedSearch),
						hash: require_utils.decodePath(hash.slice(1)),
						state: require_utils.replaceEqualDeep(previousLocation?.state, state)
					};
				}
				const url = require_rewrite.executeRewriteInput(this.rewrite, new URL(href, this.origin));
				...
				return {
					...
					publicHref: href,
					pathname: require_utils.decodePath((0, _tanstack_history.normalizeProtocolRelative)(url.pathname)),
					external: !!this.rewrite && isExternalUrl(url, this.origin),
					searchStr,
					search: require_utils.nullReplaceEqualDeep(previousLocation?.search, parsedSearch),
					hash: require_utils.decodePath(url.hash.slice(1)),
					state: require_utils.replaceEqualDeep(previousLocation?.state, state)
				};
			};
		...
		this.buildLocation = (opts) => {
			...
			const build = (dest = {}) => {
				...
				if (process.env.NODE_ENV !== "production" && dest.from && dest._isNavigate) {
					...
					const matchedFrom = require_utils.findLast(matchedRoutes, (d) => {
				...
				const nextTo = require_path.resolvePath(to[0] === "/" ? "" : dest.unsafeRelative === "path" ? current().pathname : dest.from ?? currentMatch()[1], to, this.options.trailingSlash, this.resolvePathCache);
				const destRoute = this.routesByPath[require_path.trimPathRight(nextTo)];
				const isTemplate = nextTo.includes("$");
				let destRoutes;
				if (destRoute) destRoutes = destRoute._branch ??= require_new_process_route_tree.buildRouteBranch(destRoute);
				...
				const interpolation = isTemplate ? destRoute?._interpolation ?? require_new_process_route_tree.parseSegments(false, { fullPath: nextTo }, 0) : void 0;
				let nextParams;
				for (const route of destRoutes) {
					...
					if (fn) {
						...
						if (!require_utils.hasKeys(nextParams)) break;
						if (nextParams === fromParams) nextParams = Object.assign(require_utils.createNull(), nextParams);
						try {
				...
				const nextPathname = opts.leaveParams ? nextTo : (0, _tanstack_history.normalizeProtocolRelative)(require_utils.decodePath(interpolation ? require_path.interpolatePath(nextTo, interpolation, nextParams, this.pathParamsDecoder) : nextTo));
				...
				if (this.rewrite) {
					...
					if (isExternalUrl(rewrittenUrl, origin)) {
						...
						external = true;
					} else publicHref = (0, _tanstack_history.normalizeProtocolRelative)(getUrlPath(rewrittenUrl));
				} else {
			...
			if (opts.mask) next.maskedLocation = build({
				...
				if (match) {
					const params = Object.assign(require_utils.createNull(), match.rawParams);
			...
			if (!(_tanstack_router_core_isServer.isServer ?? this.isServer) && !usedCurrent && opts._fromLocation && !next.maskedLocation) this.staticLocations.set(opts, next);
			return next;
		};
		this.commitLocation = async ({ viewTransition, ignoreBlocker, ...next }) => {
			if (_tanstack_router_core_isServer.isServer ?? this.isServer) return;
			const nextLocation = next.maskedLocation ?? next;
			if (nextLocation.external && !(_tanstack_router_core_isServer.isServer ?? this.isServer)) return documentNavigation(this, nextLocation.publicHref, {
		...
		this.buildAndCommitLocation = ({ replace, resetScroll, hashScrollIntoView, viewTransition, ignoreBlocker, ...rest } = {}) => {
			if (_tanstack_router_core_isServer.isServer ?? this.isServer) return Promise.resolve();
		...
		this.navigate = async ({ to, reloadDocument, href, publicHref, ...rest }) => {
			if (_tanstack_router_core_isServer.isServer ?? this.isServer) return;
			const hrefScheme = href ? require_utils.getUrlScheme(href) : void 0;
		...
		this.resolveRedirect = (redirect) => {
			...
			let scheme;
			if (require_utils.protocolRelativePrefixRegex.test(href) || (scheme = require_utils.getUrlScheme(href)) && !this.protocolAllowlist.has(scheme)) throw new Error(process.env.NODE_ENV !== "production" ? `Redirect blocked: unsafe protocol in href "${href}". Allowed protocols: ${Array.from(this.protocolAllowlist).join(", ")}.` : "Redirect blocked: unsafe protocol");
		...
		this.matchRoute = (location, opts) => {
			const matchLocation = {
				...location,
				to: location.to ? require_path.resolvePath(location.from || "", location.to, this.options.trailingSlash, this.resolvePathCache) : void 0,
			...
			if (location.params) {
				if (!require_utils.deepEqual(match.rawParams, location.params, true)) return false;
			}
			if (opts?.includeSearch ?? true) return require_utils.deepEqual(baseLocation.search, next.search, true) ? match.rawParams : false;
			return match.rawParams;
		};
		this.getStoreConfig = getStoreConfig;
		if (options.pathParamsAllowedCharacters?.length) this.pathParamsDecoder = require_path.compileDecodeCharMap(options.pathParamsAllowedCharacters);
		this.update({
	...
	setRoutes(caches) {
		...
		if (notFoundRoute) {
			...
			if (this.routesById[notFoundRoute.id] !== notFoundRoute) notFoundRoute._interpolation = require_new_process_route_tree.parseSegments(false, notFoundRoute, 0);
	...
	matchRoutesInternal(next, opts) {
		...
		for (let index = 0; index < matchedRoutes.length; index++) {
			...
			const usedParams = require_utils.createNull();
			const interpolatedPath = route._interpolation ? require_path.interpolatePath(route.fullPath, route._interpolation, rawParams, this.pathParamsDecoder, usedParams) : route.fullPath;
...
async function documentNavigation(router, href, { replace, ignoreBlocker }) {
	if (require_utils.isDangerousProtocol(href, router.protocolAllowlist)) {
...
function needsInheritedParams(spec, interpolation) {
	...
	return spec === void 0 || spec === true || interpolation.some((part) => typeof part !== "string" && !require_utils.hasOwn.call(spec, part[1]));
}
...
exports.lifecycleEnd = lifecycleEnd;
