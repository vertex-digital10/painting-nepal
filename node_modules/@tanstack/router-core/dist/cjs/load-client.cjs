...
function commitMatches(router, tx, matches, resolvedPrefix) {
	...
	const nextEnd = router._lifecycleEnd = require_router.lifecycleEnd(matches);
	...
	require_router.runRouteLifecycle(router, previous, matches, previousEnd, nextEnd, tx);
}
/**
...
async function hydrate(router) {
	...
	router._lifecycleEnd = require_router.lifecycleEnd(committedMatches);
