...
var BaseRoute = class {
	...
	constructor(options) {
		this.init = (originalIndex) => {
			...
			const id = isRoot ? require_root.rootRouteId : require_path.cleanPath((this.parentRoute.id === "__root__" ? "" : this.parentRoute.id) + "/" + (customId ?? ""));
			if (path === "__root__") path = "/";
			const fullPath = id === "__root__" ? "/" : path === void 0 ? this.parentRoute.fullPath : require_path.cleanPath(this.parentRoute.fullPath + "/" + path);
			this._path = path;
			this._id = id;
