...
function getRouteFilePathsFromModules(modules) {
	...
	for (const mod of modules) {
		...
		const routeFilePath = toPosixPath(mod.nameForCondition() ?? resourcePart.slice(0, queryIndex));
...
*/
function normalizeRspackClientBuild(compilation, inlineCssEnabled = false) {
...
*/
function registerClientBuildCapture(api, getConfig) {
	let clientBuild;
	api.processAssets({
		stage: "report",
		environments: [RSBUILD_ENVIRONMENT_NAMES.client]
	}, (context) => {
		clientBuild = normalizeRspackClientBuild(context.compilation, api.context.action !== "dev" && getConfig().startConfig.server.build.inlineCss.enabled);
	});
